...
#etwd("C:/Users/ACER/ariefb/SharedAssets/wsl2/Rshinny/NPname2025") #WINDOWS ONLY
#setwd("/home/unixcuon/SharedAssets/wsl2/Rshinny/NPname2025") #LINUX ONLY
getwd()

# ============================
# npnameyrapp v1.0 — Wildlife Monitoring Tool (App2)
# ============================
...
server <- function(input, output, session) {
  ...
  observeEvent(input$load_app1, {
    ...
    tryCatch({
      ...
      # Preview tables
      output$rec_preview <- renderDT({ head(rec, 100) })
      output$camop_preview <- renderDT({ head(camop, 100) })
      output$survey_dates_preview <- renderDT({
        if (!is.null(Report$survey_dates)) head(Report$survey_dates, 100) else data.frame(Note = "No survey_dates found")
      })
  ...
  observeEvent(Dfsubset(), {
    ...
    updateSelectInput(session, "kernel_species", choices = sp_choices)
  })
  ...
  # Keep your existing species input observer and download handler unchanged
  # ... (rest of your existing code)
  
  
  # Keep your existing species input table observer
  observeEvent(Dfsubset(), {
    req(Dfsubset())
    
    # Extract unique species tags
    sp_tags <- sort(unique(Dfsubset()$Species))
    
    # Create a 2-column data frame: SPname_TAG (fixed), Species_input (blank)
    ...
    output$species_input_table <- renderRHandsontable({
      rhandsontable(species_input_df, rowHeaders = NULL) %>%
        hot_col("SPname_TAG", readOnly = TRUE)  # lock left column
    })
  })
  ...
  # Keep your existing download handler
  output$download_gbif_tagged <- downloadHandler(
    ...
    content = function(file) {
      ...
      outFile <- file.path(outDir, basename(file))  # use generated filename
      
      # Save to outDir
      tryCatch({
        openxlsx::write.xlsx(gbif_tagged(), outFile, overwrite = TRUE)
      ...
      # Copy to Shiny temp file for download
  ...
  observeEvent(input$run_kernel, {
    ...
    if (mode == "Single Species") {
      ...
      species_info <- isolate(species_events()) %>% filter(n_events >= 2)
    ...
    # In your server, add this download handler
    output$download_kernel_plot <- downloadHandler(
      filename = function() {
        mode <- if (kernel_data$plot_data$mode == "Single Species") "singleSP" else "allSP"
        center <- if (kernel_data$plot_data$center == "Daylight") "noon" else "midnight"
        sp_name <- if (mode == "singleSP") kernel_data$plot_data$selected_sp else ""
        timestamp <- format(Sys.time(), "%Y%m%d-%H%M")
        paste0(mode, "-", center, "-", sp_name, "-", timestamp, ".png")
      },
      content = function(file) {
        # Create the exact same plot as in preview
  ...
  output$download_occupancy_summary <- downloadHandler(
    ...
    content = function(file) {
      if (input$download_format == "xlsx") {
        ...
        if (input$occupancy_mode == "Single Species") {
          sheet1 <- xlsx::createSheet(wb, sheetName = "Summary")
          sheet2 <- xlsx::createSheet(wb, sheetName = "AIC Table")
          xlsx::addDataFrame(occupancy_summary(), sheet1, row.names = FALSE)
          xlsx::addDataFrame(aic_table_preview(), sheet2, row.names = FALSE)
        } else {
          sheet1 <- xlsx::createSheet(wb, sheetName = "All Species Summary")
          sheet2 <- xlsx::createSheet(wb, sheetName = "All Species AIC")
          xlsx::addDataFrame(summary_table_all_species(), sheet1, row.names = FALSE)
          xlsx::addDataFrame(aic_table_preview(), sheet2, row.names = FALSE)
        }
